...
router.get(`/`, async (req, res) => {
  try {
    const userList = await User.find().select('-passwordHash -image -imgStore -photoProof');
    ...
    const formatteduserList = userList.map(user => {
      return {
        ...
        photoProof: user.photoProof ? `/pbl6/user/photoProof/${photoProof.id}` : null, // Đường dẫn đến route mới
        store: user.store,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    upload(req, res, async (err) => {
      ...
      if (req.files && req.files.photoProof) {
        const isValid = FILE_TYPE_MAP[req.files.photoProof[0].mimetype];
        if (!isValid) {
          return res.status(400).send('Invalid image type for photoProof');
        }

        updatedFields.photoProof = {
          data: req.files.photoProof[0].buffer,
          contentType: req.files.photoProof[0].mimetype
        };
      }
...
router.post(`/register`, uploadOptions.fields([{ name: 'image', maxCount: 1 }, { name: 'imgStore', maxCount: 1 },
{ name: 'photoProof', maxCount: 1 }]), async (req, res) => {
  try {
...
router.get('/photoProof/:id', async (req, res) => {
  try {
    const user = await User.findById(req.params.id);

    if (!user || !user.photoProof) {
      return res.status(404).json({ success: false, message: 'Image not found' });
    }

    // Đặt loại nội dung và gửi dữ liệu hình ảnh
    res.contentType(user.photoProof.contentType);
    res.send(user.photoProof.data);
  } catch (error) {
    console.error('Error retrieving image:', error);
    res.status(500).send('Internal Server Error');
  }
